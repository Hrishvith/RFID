...
function LoginForm({ onForgotPassword }) {
  ...
  async function handleSubmit(e) {
    ...
    setLoading(true);
    try {
      await login({ email, password, rememberMe, portal: "user" });
  ...
  return (
    <>
      ...
      <form onSubmit={handleSubmit} className="mt-8 space-y-5">
        ...
        <div className="space-y-1.5 pt-1 text-center text-sm text-blue-100/70">
          <p>
      ...
      {sent ? (
        ...
        <form onSubmit={handleSubmit} className="mt-8 space-y-5">
          ...
          <button
            type="submit"
            disabled={loading}
            className="flex h-12 w-full items-center justify-center rounded-xl bg-[#2563EB] text-[15px] font-semibold text-white shadow-lg shadow-blue-900/30 transition hover:bg-[#1D4ED8] disabled:cursor-not-allowed disabled:opacity-60"
          >
